...
#disambiguate! 
const IUPACbool = Dict{Char,Vector{Bool}}()
...
resolve_seq(s) = join(resolve_base.(collect(s)))

#encode string
function string2encoding(seq::String, lookup_dic::AbstractDict{Char,Int}; missing_chars = Set{Char}())
    vec = Int[]
    for c in seq
